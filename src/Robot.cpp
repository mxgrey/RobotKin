...
void Robot::initialize(vector<Linkage> linkages, vector<int> parentIndices)
{
    ...
    for (size_t i = 0; i != linkages.size(); ++i) {
        ...
        Linkage* tempLinkage = new Linkage(linkages[iPI[i].I]);
        ...
        if (parentIndices[iPI[i].I] == -1) {
            ...
            cerr << "Linkage " << linkages_[i]->name_ << " (" << i << ") is given a parent of "
                 << linkages_[parentIndices[iPI[i].I]]->name_ << " (" << iPI[i].I << ")" << endl;
        }
